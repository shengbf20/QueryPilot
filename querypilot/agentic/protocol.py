"""Tool-call protocol for the strong-agent path."""
...
- run_sql：执行一条只读 SQL。args.sql 必填。提交的必须是能直接回答问题的最终语句。
- ask_user：无法写出唯一合理 SQL 时追问。args.message 点明缺口。
- finish：结束。只有确认结果表严格满足题面后才 finish；「执行成功」不等于答对。
...
15. 年龄段等分桶标签用简洁区间 <30、[30,50)、[50,60)、[60,)（「大于60」对应 [60,)），不要写成 >=60，不要加「1.」「2.」序号前缀。分桶只输出有数据的组，禁止用 UNION/维表把空档补成 0。
...
17. 问及「盈亏/损益」时禁止臆造盈亏表；用资产快照+ dws_cust_fin_d 推算，投影六列：pty_id, bgn_aset, end_aset, aset_in, aset_out, aset_pft。26年Q1 期初 data_dt='20260101'、期末 '20260331'；aset_in/out 对 cash/tran/assign 流入流出求和。aset_pft 必须写成 end_nm+end_fc - (bgn_nm + bgn_fc) + aset_out - aset_in（期初 nm+fc 必须加括号一起减）；禁止写成 -bgn_nm + bgn_fc。六列数值外层一律 coalesce(...,0)。
...
参考示例只借口径；本题多出来的条件必须自己补上，不要照抄未使用的 CTE。问句含糊时用 ask_user，不要猜测 SQL。

输出：先写几句中文判断，再给出一个或多个工具 JSON，每个单独一行。

示例（意图清楚，Schema 已在首轮给出）：
{"tool": "run_sql", "args": {"sql": "SELECT COUNT(*) AS cnt FROM ads_cust_info_d WHERE cust_age > 30"}}

对照结果后结束：
{"tool": "finish", "args": {}}

问句含糊：
{"tool": "ask_user", "args": {"message": "要统计人数、资产还是持仓？"}}
"""


def parse_tool_call(data: dict[str, Any]) -> tuple[str, dict[str, Any], str]:
    """Extract (tool, args, thought) from a parsed object."""
    name = str(data.get("tool") or data.get("name") or "").strip()
    if name not in TOOL_NAMES:
        raise ValueError(f"unknown or missing tool: {name or '(empty)'}")
    raw_args = data.get("args") or data.get("arguments") or {}
    if not isinstance(raw_args, dict):
        raw_args = {}
    thought = str(data.get("thought") or data.get("rationale") or "").strip()
    return name, raw_args, thought


def parse_agent_turn(text: str) -> tuple[str, dict[str, Any], str]:
    """Parse a turn: optional prose, then the first tool JSON object."""
    calls = parse_agent_tools(text)
    name, args, thought = calls[0]
    return name, args, thought


def parse_agent_tools(text: str) -> list[tuple[str, dict[str, Any], str]]:
    """Parse one or more tool calls from a model turn (document order)."""
    blob = (text or "").strip()
    if not blob:
        raise ValueError("empty model response")
    found = _extract_all_tool_json(blob)
    if not found:
        raise ValueError("no tool call JSON found")
    calls: list[tuple[str, dict[str, Any], str]] = []
    for i, (data, start) in enumerate(found):
        name, args, thought_in_json = parse_tool_call(data)
        preamble = blob[:start].strip() if i == 0 else ""
        for marker in ("```json", "```"):
            if preamble.endswith(marker):
                preamble = preamble[: -len(marker)].rstrip()
        calls.append((name, args, preamble or thought_in_json))
    return calls


def _extract_all_tool_json(text: str) -> list[tuple[dict[str, Any], int]]:
